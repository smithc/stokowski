...
All state lives in memory. The orchestrator recovers from restart by re-polling Linear and re-discovering active issues. Workspace directories on disk act as durable state. The single narrow exception is session-id persistence (see below): the one piece of state that has no Linear mirror and cannot be re-derived from the workspace.

### Session-id persistence
Claude Code session ids (`_last_session_ids`) persist to a single JSON file so inherit-mode states resume their prior conversation across orchestrator restarts AND mid-turn kills. Without this, every in-flight state would start a fresh Claude session on re-dispatch, losing tool-use history and reasoning context.

- **Default on.** The feature is enabled by default with path `{primary workspace.root}/.stokowski-sessions.json`. Operators override via `session_persistence.path` or disable via `session_persistence.enabled: false`.
- **Multi-project: one file per orchestrator.** Issue ids are globally unique across Linear workspaces, so a single file covers every project. The primary project's `workspace.root` determines the default location.
- **Mid-turn capture.** `_process_event` in `runner.py` captures `session_id` from any event that carries it — including Claude Code's early `system`/`init` event, not just the final `result` event. When captured, the runner fires the `on_session_id` callback, and the orchestrator writes through to `_last_session_ids` + `SessionStore` immediately. This means a `claude -p` subprocess killed mid-turn (SIGKILL, orchestrator crash, operator pause) still leaves a resumable session id on disk — the next dispatch resumes the same Claude session via `--resume`, reading Claude Code's own on-disk transcript file. Without this, the mid-turn conversation would be lost.
- **`session: fresh` suppression is preserved at both save sites.** The fresh-flag check at `_on_worker_exit` gates the end-of-turn write. The `_build_session_id_callback` method returns `None` for fresh-mode states, so mid-turn capture is disabled too — fresh session ids never touch disk regardless of when they're observed.
- **Codex is unaffected.** `run_codex_turn` doesn't emit session ids (Codex has no resumption concept), so this entire layer is Claude-Code-only.
- **Best-effort.** Load tolerates missing files and corruption (starts with an empty map + WARN log). Write failures on save/evict/mid-turn are caught and logged; the orchestrator degrades to pre-feature behavior rather than crashing.
- **Atomic writes.** `SessionStore._write_atomic` writes to a sibling tmp file then `os.replace` — no partial-write corruption windows.
- **Eviction.** `_cleanup_issue_state(issue_id)` and `_startup_cleanup` (for terminal issues) both evict from the store, keeping the file sized to real active work.
- **Schema versioned.** File format is `{"version": 1, "sessions": {issue_id: session_id, ...}}`. Bump `_SCHEMA_VERSION` in `stokowski/session_store.py` if the on-disk shape ever changes.
...
- `SessionPersistenceConfig` — Claude Code session-id persistence across restarts: `enabled` (default **true**), `path` (supports `~`, `$VAR`, and relative-to-workflow-dir; defaults to `{primary workspace.root}/.stokowski-sessions.json`). `resolved_path(workspace_root, workflow_dir)` resolves the final path. See **Session-id persistence** under *Key design decisions*.
...
- **Session-id persistence**: `SessionStore` persists `_last_session_ids` to a JSON file so inherit-mode states resume across restarts AND mid-turn kills. Two save sites: end-of-turn at `_on_worker_exit` and mid-turn via the `on_session_id` runner callback built by `_build_session_id_callback`. BOTH apply the `session: fresh` suppression — the callback returns `None` for fresh-mode states so the runner doesn't even call it. Never remove the fresh-flag check at either site, or fresh sandbox sessions will poison the inherit chain. Write failures are logged-and-swallowed: session persistence is best-effort and MUST NOT crash the orchestrator tick. Safe to delete the file manually — missing file is the bootstrap state and falls back to fresh-on-restart behavior.
